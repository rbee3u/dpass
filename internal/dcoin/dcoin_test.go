...
func TestEntropyToMnemonic(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if err != nil {
			t.Fatalf("failed to convert entropy(%s): %v", tt.entropy, err)
		}
		if mnemonic != tt.mnemonic {
			t.Errorf("mnemonic: got = %s, want = %s", mnemonic, tt.mnemonic)
		}
	}
}
...
func TestMnemonicToSeed(t *testing.T) {
	...
	for _, tt := range tests {
		seed, err := dcoin.MnemonicToSeed(tt.mnemonic, "TREZOR")
		if err != nil {
			t.Fatalf("failed to convert mnemonic(%s): %v", tt.mnemonic, err)
		}
		if seed := hex.EncodeToString(seed); seed != tt.seed {
			t.Errorf("seed: got = %s, want = %s", seed, tt.seed)
		}
	}
}
...
func TestSeedToKey(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if err != nil {
			t.Fatalf("failed to decode seed(%s): %v", tt.seed, err)
		}
		key, err := dcoin.SeedToKey(seed, tt.path)
		if err != nil {
			t.Fatalf("failed to derive key from seed(%x): %v", seed, err)
		}
		if pub := key.PublicKey().String(); pub != tt.pub {
			t.Errorf("pub: got = %s, want = %s", pub, tt.pub)
		}
		if prv := key.String(); prv != tt.prv {
			t.Errorf("prv: got = %s, want = %s", prv, tt.prv)
		}
	}
}
