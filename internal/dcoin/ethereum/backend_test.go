...
func TestBackend(t *testing.T) {
	...
	for _, tt := range tests {
		address, err := (&backend{network: tt.network, index: tt.index}).getResult(mnemonic)
		if err != nil {
		...
		private, err := (&backend{network: tt.network, index: tt.index, secret: true}).getResult(mnemonic)
		if err != nil {
