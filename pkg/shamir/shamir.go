...
// MinShares is the smallest valid threshold and the fewest shares required before
// attempting polynomial interpolation.
const MinShares = 2
...
// Constraints: MinShares <= threshold <= parts <= MaxShares. Invalid inputs fail
// with SplitConstraintError. Each share is len(secret)+1 bytes long; the last byte
// stores its unique non-zero x-coordinate, and the preceding bytes store the
// evaluated secret polynomial values. The returned share format does not encode
// threshold, so callers that persist or transport shares must track that value
// separately before calling Combine.
...
// Combine reconstructs the secret from shares produced by Split. It validates only
// the share encoding: callers must provide at least two shares of the same length,
// each at least MinShareLength bytes long, with a distinct non-zero x-coordinate
// stored in the last byte. The share format does not encode the original
// threshold, so Combine cannot verify that enough shares were provided. Callers
// must enforce that precondition before calling; otherwise Combine may return
// incorrect data without reporting an error.
...
// Caller must ensure degree >= 0. Coefficients are ordered from x^0 upward; when
// degree > 0, the leading coefficient is guaranteed non-zero.
func polyGenerate(degree int, intercept uint8) []uint8 {
	coefficients := make([]uint8, degree+1)
	coefficients[0] = intercept
	if degree > 0 {
		// crypto/rand.Read always returns len(p), nil since Go 1.20; it panics instead of
...
// polyWeights returns the Lagrange basis weights for interpolating the polynomial
// defined by xCoordinates at x. Caller must provide distinct x-coordinates.
...
// fieldAdd returns x + y in GF(2^8). In characteristic 2, addition equals bitwise
// XOR and subtraction coincides with addition, so there is no fieldSub.
func fieldAdd(x, y uint8) uint8 {
	return x ^ y
}

// fieldMul returns x * y in GF(2^8) via constant-time multiplication reduced by
// the AES polynomial x^8+x^4+x^3+x+1 (0x11b); 0x1b is its low 8 bits, XOR'd in at
// each left-shift step for modular reduction.
...
// fieldInv returns x^(-1) in GF(2^8), the unique y satisfying x * y == 1. Caller
// must ensure x != 0. By Fermat's little theorem x^(-1) = x^254; the addition
// chain below builds up to x^127, then squares it to obtain x^254.
